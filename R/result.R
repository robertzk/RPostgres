...
setMethod("dbFetch", "PqResult", function(res, n = -1, ..., row.names = NA) {
  ...
  if (existsFunction("sqlColumnToRownames", where = getNamespace("DBI"))) {
    getFromNamespace("sqlColumnToRownames", "DBI")(result_fetch(res@ptr, n = n), row.names)
  } else if (existsFunction("columnToRownames", where = getNamespace("DBI"))) {
    getFromNamespace("columnToRownames", "DBI")(result_fetch(res@ptr, n = n), row.names)
  } else {
    stop("Unexpected DBI error: ensure columnToRownames or sqlColumnToRownames exists.")
  }
})
